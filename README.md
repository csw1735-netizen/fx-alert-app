...
- 기존에 있던 `TELEGRAM_BOT_USERNAME`, `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`는 더 이상 필요 없으니
  삭제해도 됩니다 (남겨둬도 동작에는 지장 없음).
...
## 5단계. 로그인해서 사용하기 (Chat ID + PIN)

1. 텔레그램에서 내 봇을 검색해 아무 메시지나 하나 보내기 (봇이 나에게 메시지를 보낼 수 있으려면 먼저
   대화가 시작되어 있어야 합니다)
2. 브라우저에서 `https://api.telegram.org/bot<봇토큰>/getUpdates` 접속 → 결과 안의 `"chat":{"id":...}` 값이
   내 Chat ID
3. 배포된 URL 접속 → Chat ID와, 처음이면 원하는 4~8자리 숫자 PIN을 정해서 "로그인 / PIN 설정" 클릭
4. 다른 기기에서도 같은 Chat ID + PIN을 입력하면 항상 내 설정 그대로 불러와집니다
5. 변동 기준(%나 원), 정기 알림(정각/5분), 주말 알림 끄기 등을 설정 — 저장은 자동으로 됨
6. "테스트 메시지 보내기"로 정상 수신 확인

PIN은 서버에 원문이 아니라 솔트를 섞은 해시로만 저장되고, 로그인 시도는 15분에 10회로 제한되어 있어
무작위 대입을 막습니다. 다른 사람이 내 Chat ID를 안다고 해도 PIN을 모르면 내 설정을 바꿀 수 없습니다.
